...
async function exerciseForcedCheckpointFailure(t, checkpointMode) {
  const root = mkdtempSync(path.join(os.tmpdir(), "modeldock-restart-checkpoint-failed-"));
  const stateDir = path.join(root, ".state");
  mkdirSync(path.join(root, "scripts"), { recursive: true });
  mkdirSync(path.join(root, "dist"), { recursive: true });
  mkdirSync(stateDir, { recursive: true });
  ...
  writeFileSync(path.join(root, "scripts", "restart.ps1"), readFileSync(path.join(repoRoot, "scripts", "restart.ps1")), "utf8");
  writeFileSync(path.join(root, "scripts", "gateway-verifier.mjs"), readFileSync(path.join(repoRoot, "scripts", "gateway-verifier.mjs")), "utf8");
  const checkpointHandler = checkpointMode === "stall"
    ? "return;"
    : `res.writeHead(503, { "content-type": "application/json" });
    res.end(JSON.stringify({ error: { type: "local_restart_checkpoint_failed", message: "stuck lane" } }));
    return;`;
  writeFileSync(entry, `
import http from "node:http";
import { mkdirSync, writeFileSync } from "node:fs";
...
`, "utf8");
  ...
  const code = await new Promise((resolve) => restart.on("close", resolve));
  assert.equal(code, 0, output);
  assert.match(output, /checkpoint failed; continuing restart without a hot-state dump/i);
  assert.match(output, /verified gateway/i);
  assert.equal(await waitForHealth(port), true, "checkpoint failure must not strand the gateway");
  assert.match(readFileSync(path.join(repoRoot, "scripts", "restart.ps1"), "utf8"), /if \(\$forceTakeover\) \{ 5 \} else \{ 30 \}/);
  return { elapsedMs: Date.now() - startedAt, output };
}

test("restart.ps1 force replaces a gateway when its local KV checkpoint returns 503", async (t) => {
  if (process.platform !== "win32") {
    t.skip("restart.ps1 is Windows-only");
    return;
  }
  await exerciseForcedCheckpointFailure(t, "503");
});

test("restart.ps1 force bounds a checkpoint that never returns and still replaces the gateway", { timeout: 20_000 }, async (t) => {
  if (process.platform !== "win32") {
    t.skip("restart.ps1 is Windows-only");
    return;
  }
  const result = await exerciseForcedCheckpointFailure(t, "stall");
  assert.ok(result.elapsedMs < 15_000, `forced restart took ${result.elapsedMs}ms`);
});
