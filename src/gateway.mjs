...
const LOCAL_PLAN_GUIDANCE = "LONG TASK PLAN (REQUIRED): If update_plan is available and the user's task has multiple steps or spans turns, call update_plan before substantial work and keep a concise plan current. Update it when progress, blockers, or the next action materially change; do not emit a status summary in every reply. Skip this for a trivial single-step answer. A plan is not authorization to act on stale data: verify current facts before acting. Do not create a goal unless the user explicitly requests a persistent goal.";
...
function appendLocalGuidance(instructions) {
  ...
  if (Array.isArray(instructions)) {
    ...
    return missing.length ? [...instructions, { type: "input_text", text: missing.join("\n") }] : instructions;
  }
  if (typeof instructions === "string") {
    const missing = guidance.filter((line) => !instructions.includes(line));
    return missing.length ? `${instructions}\n${missing.join("\n")}` : instructions;
  }
...
async function prepareRoutedInput(payload, services, route, signal) {
  ...
  // Both compact and ordinary local relays must see the same bounded history.
  // Codex retains user items across compact_v2, including repeated scheduled
  // prompts; folding only the CPU summary left the next llama request intact.
  if (isLocalBackend(config, route.model)) input = projectLocalHistory(input);
