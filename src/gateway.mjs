...
function localWarmBaseFor({ payload, target, upstreamModel }) {
  let chat;
  try {
    chat = responsesToChat(payload).payload;
  } catch {
    return null;
  }
  ...
  const messages = Object.freeze([
    ...
    Object.freeze({ role: "assistant", content: LOCAL_WARM_BOOTSTRAP_ASSISTANT }),
  ]);
  return Object.freeze({
    ...
    async create({ slot, signal }) {
      ...
      return Boolean(
        message
          && message.content?.trim?.() === LOCAL_WARM_BOOTSTRAP_ASSISTANT
...
function injectLocalWarmBase(chatPayload, warmBase) {
  if (!warmBase || !Array.isArray(chatPayload?.messages) || chatPayload.messages[0]?.role !== "system") return chatPayload;
  return {
    ...chatPayload,
