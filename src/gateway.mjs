...
const LOCAL_WARM_BOOTSTRAP_USER = "Reply with exactly BOOTSTRAP_READY. Do not call a tool.";
const LOCAL_WARM_BOOTSTRAP_ASSISTANT = "BOOTSTRAP_READY";
const LOCAL_WARM_BOOTSTRAP_VERSION = 1;
const LOCAL_WARM_BOOTSTRAP_MAX_TOKENS = 256;
...
// llama.cpp's Chat route caches an entire completed chat turn, not a bare
// system/tools prefix. A deterministic synthetic user/assistant exchange gives
// new Codex sessions a valid append boundary: the first real user message can
// follow it exactly like an ordinary continuation. The identity intentionally
// hashes the exact serialized prefix inputs; a harmless cache miss is safer
// than reusing a slot after a tool, template, or slim-instruction change.
function localWarmBaseFor({ payload, target, upstreamModel }) {
  ...
  const signature = createHash("sha256").update(JSON.stringify({
    version: LOCAL_WARM_BOOTSTRAP_VERSION,
    // This is the model identity Codex selected. llama.cpp can change its
    // advertised wire id from a friendly name to the GGUF path during a boot
    // rescan without changing the loaded model; that spelling change must not
    // create a second otherwise-identical base KV. A real launch/model change
    // is covered by the managed host fingerprint that gates the KV store.
    model: payload.model,
  ...
  const sessionKey = createHash("sha256").update(`modeldock:warm-base:${signature}`).digest("hex");
  const messages = Object.freeze([
    Object.freeze({ role: "user", content: LOCAL_WARM_BOOTSTRAP_USER }),
  ]);
  return Object.freeze({
    sessionKey,
    messages,
    requiresTranscript: true,
    async create({ slot, signal }) {
      const response = await fetch(target.url, {
        method: "POST",
        headers: upstreamHeaders(target),
        body: JSON.stringify({
          model: upstreamModel,
          messages: [system, messages[0]],
          stream: false,
          ...(Number.isSafeInteger(slot) && slot >= 0 ? { id_slot: slot } : {}),
          ...(chat.tools?.length ? { tools: chat.tools } : {}),
          ...(chat.chat_template_kwargs ? { chat_template_kwargs: chat.chat_template_kwargs } : {}),
          max_tokens: LOCAL_WARM_BOOTSTRAP_MAX_TOKENS,
          temperature: 0,
          cache_prompt: true,
        }),
        signal,
      ...
      if (!message || message.content?.trim?.() !== LOCAL_WARM_BOOTSTRAP_ASSISTANT || message.tool_calls?.length) return null;
      const assistantReasoningContent = typeof message.reasoning_content === "string" ? message.reasoning_content : "";
      return Object.freeze({
...
function injectLocalWarmBase(chatPayload, warmBase) {
  if (!warmBase || !Array.isArray(warmBase.messages) || warmBase.messages.length < 2 || !Array.isArray(chatPayload?.messages) || chatPayload.messages[0]?.role !== "system") return chatPayload;
  return {
    ...chatPayload,
    messages: [chatPayload.messages[0], ...warmBase.messages, ...chatPayload.messages.slice(1)],
  };
}
...
export async function relayResponses(payload, res, services, { signal } = {}) {
  ...
  try {
    if (target.provider === "llamacpp" && services.localHostRuntime) {
      return await services.localHostRuntime.run({ sessionId, threadId, signal, warmBase: localWarmBase, run: executeRelay });
    }
