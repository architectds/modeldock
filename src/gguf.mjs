...
// Bytes per block and elements per block, per ggml tensor type. Quantized
// types pack a fixed number of weights into a block that carries its own
// scales, so a tensor's size is (elements / blockElements) * blockBytes rather
// than anything proportional to a single element.
const GGML_TYPES = {
  0: [4, 1], 1: [2, 1], 2: [18, 32], 3: [20, 32], 6: [22, 32], 7: [24, 32],
  8: [34, 32], 9: [36, 32], 10: [84, 256], 11: [110, 256], 12: [144, 256],
  13: [176, 256], 14: [210, 256], 15: [292, 256], 16: [84, 256], 17: [110, 256],
  18: [84, 256], 19: [66, 256], 20: [74, 256], 21: [98, 256], 24: [2, 1],
  25: [34, 256], 26: [18, 32], 27: [20, 32], 28: [1, 1], 29: [2, 1],
  30: [4, 1], 31: [8, 1],
};

// Every key/value pair in the header, and what each tensor costs.
//
// The tensor directory sits immediately after the metadata, so reading it is
// the same file read: the cursor is already there. It is worth reading because
// the file's own size is not what a model costs on the card. A GGUF carrying
// multi-token-prediction blocks ships them in the file and the backend logs
// "-- ignoring" and never loads them, so sizing against stat().size charges the
// card for weights it does not hold - measured at 221.7 MB across the 15
// blk.64.* tensors of Qwen3.8-27B-Q3_K_M.
export function readGgufMetadata(file) {
  ...
  try {
    ...
    for (let i = 0; i < tensorCount; i += 1) {
      const name = c.raw(c.u64());
      const dims = c.u32();
      let elements = 1;
      for (let d = 0; d < dims; d += 1) elements *= c.u64();
      const type = c.u32();
      c.u64(); // offset into the data section, which nothing here needs
      const spec = GGML_TYPES[type];
      // An unknown type means a newer quantization than this table knows. The
      // metadata is still good, so the header is returned without a tensor
      // ledger rather than thrown away - callers fall back to the file size.
      if (!spec) return { version, tensorCount, meta, tensors: null };
      tensors.push({ name, bytes: (elements / spec[1]) * spec[0] });
    }
    return { version, tensorCount, meta, tensors };
  } finally {
    closeSync(fd);
  }
}

// What the backend actually loads, and what it leaves in the file.
//
// Blocks numbered at or past the layer count are the multi-token-prediction
// blocks: `layers` is already block_count minus nextn_predict_layers, so the
// ones past it are exactly the blocks that carry weights no KV cache and no
// forward pass will use.
export function tensorWeights(tensors, layers) {
  if (!Array.isArray(tensors) || !layers) return null;
  let loaded = 0;
  let ignored = 0;
  for (const tensor of tensors) {
    const block = /^blk\.(\d+)\./.exec(tensor.name);
    if (block && Number(block[1]) >= layers) ignored += tensor.bytes;
    else loaded += tensor.bytes;
  }
  return { loaded: Math.round(loaded), ignored: Math.round(ignored) };
}
...
// machine: a measured 18.70 GiB total at 81920 tokens, against 12.6429 GiB of
// weights and 5.00 GiB of computed KV, leaves 1.0571 GiB. Callers can override
// it as better measurements land.
//
// It was 0.83 GiB while the weights term was the file's own size. That term is
// now the tensors the backend loads, which is 221.7 MB smaller on this model,
// and the two numbers move together because they come out of ONE measurement:
// overhead is what the total has left after weights and KV. Correcting the
// weights alone would have quietly shrunk the estimate by that 221.7 MB, which
// is the direction that recommends a context the card cannot hold. Reconciled:
// 12.6429 + 5.0000 + 1.0571 = 18.70, and with the old pair
// 12.8697 + 5.0000 + 0.8303 = 18.70 - the same total, split honestly.
//
...
export const DEFAULT_OVERHEAD_BYTES = Math.round(1.0571 * 1024 ** 3);
...
export function readModelFacts(file) {
  ...
  const { meta, tensors } = readGgufMetadata(file);
  ...
  return {
    path: file,
    fileBytes: stat.size,
    // What the card is asked to hold, which is not the same as what the file
    // weighs: the tensor directory minus the blocks the backend skips, and
    // minus the header and the alignment padding that never reach the GPU.
    // Falls back to the file size when the ledger could not be read, which is
    // the figure this used before and errs on the large side.
